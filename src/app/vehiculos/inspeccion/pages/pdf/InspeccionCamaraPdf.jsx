...
export default function InspeccionCamaraPdf() {
  ...
  useEffect(() => {
    const loadInspection = async () => {
      try {
        ...
        if (error || !data) {
          console.error("Error cargando PDF cámara:", error);
          setInspection(null);
          return;
        }
        ...
        console.error("Error inesperado cargando PDF cámara:", err);
        setInspection(null);
      } finally {
        setLoading(false);
      }
  ...
  if (!inspection) {
    return (
      <div className="p-6">
        <p>No se encontró la inspección de cámara.</p>
        <button
  ...
  const handlePrint = () => {
    ...
    document.title = `${cliente}_${pedido}_${codigo}_Inspeccion_Camara_ASTAP`;
    window.print();
          ...
          <table className="pdf-table">
            <tbody>
              <tr>
                ...
                <td style={{ width: 180, fontSize: 10 }}>
                  <div>
          ...
          <table className="pdf-table mt-4">
            <tbody>
              {[
                ...
                ["FECHA DE SERVICIO", data.fechaServicio || data.fechaInspeccion],
              ].map(([l, v], i) => (
                <tr key={i}>
            ...
            <tbody>
              <tr>
                <td colSpan={2} style={{ position: "relative" }}>
                  <img
                    src={data.estadoEquipoImagenUrl || "/estado equipo camara.png"}
                    style={{ width: "100%" }}
