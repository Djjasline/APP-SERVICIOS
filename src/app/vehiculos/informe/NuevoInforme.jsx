      ...
      const cleanData = {
        ...
        actividades: Array.isArray(report.data?.actividades)
          ? report.data.actividades.map((a) => ({
              ...
              contractItemsTable: normalizeContractItemsTable(a?.contractItemsTable),
            }))
          : [{ titulo: "", detalle: "", imagenes: [], contractItemsTable: null }],
        firmas: {
...
const updateEstadoEquipoPointObservation = (imageId, pointId, value) => {
  ...
  const addContractItemsTable = (actividadIndex) => {
...
const validateReport = () => {
  ...
  const actividadValida = data.actividades.some((a) => {
    const detalleValido = a.titulo?.trim() && (a.detalle || "").trim().length >= 15;
    return detalleValido || hasContractItemsTableData(a.contractItemsTable);
  });
              ...
              <tr key={i}>
                ...
                <td className="align-top">
                  ...
                  {a.contractItemsTable ? (
                    ...
                    <button
                      type="button"
                      onClick={() => addContractItemsTable(i)}
                      className="mt-3 inline-flex items-center justify-center rounded-md border border-blue-300 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-800 shadow-sm transition hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"
                    >
                      Insertar tabla tipo Excel
                    </button>
                  )}
