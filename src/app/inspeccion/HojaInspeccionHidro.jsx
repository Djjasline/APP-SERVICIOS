...
export default function HojaInspeccionHidro() {
  ...
  const endDraw = (ref, field) => {
    ...
    setFormData((p) => ({
      ...p,
      [field]: ref.current.toDataURL("image/png"),
    }));
  };
        ...
        <div className="grid grid-cols-2 gap-4 text-xs text-center">
          ...
          <div>
            ...
            <canvas
              ...
              onPointerDown={(e) => startDraw(e, canvasTecnico)}
              onPointerMove={(e) => draw(e, canvasTecnico)}
              onPointerUp={() => endDraw(canvasTecnico, "firmaTecnico")}
            />
            <button type="button" onClick={() => clearCanvas(canvasTecnico, "firmaTecnico")} className="text-red-600 text-xs mt-1">
              Limpiar
            </button>
          </div>
          ...
          <div>
            ...
            <canvas
              ...
              style={{ touchAction: "none" }}
              className="border w-full bg-white"
              onPointerDown={(e) => startDraw(e, canvasCliente)}
              onPointerMove={(e) => draw(e, canvasCliente)}
              onPointerUp={() => endDraw(canvasCliente, "firmaCliente")}
            />
            <button type="button" onClick={() => clearCanvas(canvasCliente, "firmaCliente")} className="text-red-600 text-xs mt-1">
              Limpiar
            </button>
          </div>
